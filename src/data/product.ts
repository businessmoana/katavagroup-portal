...
export const useDeleteOrderItemlMutation = () => {
  ...
  return useMutation(productClient.deleteOrderItem, {
    onSuccess: () => {
      toast.success(t('common:successfully-deleted'));
    },
    // Always refetch after error or success:
    onSettled: () => {
      queryClient.invalidateQueries(API_ENDPOINTS.PICKPACK);
    },
  });
};

export const useDeleteOrderlMutation = () => {
  const queryClient = useQueryClient();
  const { t } = useTranslation();

  return useMutation(productClient.deleteOrder, {
    onSuccess: () => {
      toast.success(t('common:successfully-deleted'));
    },
    // Always refetch after error or success:
