...
export interface KorisnickiNalog {
  ...
  lozinka: string,
  datum_otvaranja_naloga: string,
  broj_logovanja: string,
  datum_poslednjeg_logovanja: string,
  pomocna: string,
  sif_uloga_id: number,
  sifUloga: SifUloga,
}

export interface SifUloga {
  id: number,
  naziv: string
}

export interface ChefLocation {
  reduce(arg0: (chefLocation: any) => any): unknown;
  forEach(arg0: (chefLocation: any) => void): any;
  map(): unknown;
  find(arg0: (chefLocation: { location: { id: number; }; }) => boolean): unknown;
  id: string,
  chef_id: number,
  location_id: number,
  location: Location,
...
export interface ChefQueryOptions extends Omit<QueryOptions, 'language'> {
  search: string;
  is_active?: boolean;
}

export interface ChefOrdersQueryOptions extends Omit<QueryOptions, 'language'> {
  search: string;
}

export interface ChefInvoicesQueryOptions extends Omit<QueryOptions, 'language'> {
  search: string;
}

export interface PriceGroupQueryOptions extends Omit<QueryOptions, 'language'> {
  search: string;
  is_active?: boolean;
}

export interface DateIntervalQueryOptions extends Omit<QueryOptions, 'language'> {
  search: string;
  is_active?: boolean;
}
...
export interface ChefPaginator extends PaginatorInfo<Chef> { }
export interface ChefOrdersPaginator extends PaginatorInfo<any> { }
export interface ChefInvoicesPaginator extends PaginatorInfo<any> { }
export interface PriceGroupPaginator extends PaginatorInfo<PriceGroup> { }
export interface DateIntervalPaginator extends PaginatorInfo<DateInterval> { }
